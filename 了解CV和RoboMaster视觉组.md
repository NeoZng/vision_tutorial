...
> 阅读这本小节内容需要对目标检测有相当的认识，请根据自身情况酌情选择。这些观点部分来源于实验验证，部分来源于笔者的测试以及猜想。若你已经对目标检测和pytorch编程较为熟悉，建议阅读。
...
1. 基于矩形框，回归出四个角点的offsets
2. 基于单个参考点，将问题看作任意四边形的检测
3. 视为特征点检测问题，不管框

区别一下这三种思路，它们的出发点是不同的。第一种思路还是经典的目标检测，先用一个外接矩形把装甲板框住，然后在框内再确定四个点的位置（可以是一阶段直接把框和点一起预测出来，也可以采用框得到ROI再加卷积得点）。第二种思路则是预测一个**四边形**而不是正放的矩形，IOU仍然是很重要的优化指标，因为这要求我们预测的bbox很好地贴合目标。第三种方法则不考虑IOU，使用的是人体关键点检测、面部关键点检测的思路，建模的是**点和点之间的关系**。装甲板不像通用目标检测，我们应该从它横平竖直的特点出发，考虑这三种思路。

在检测头的代码设计上这三种都没什么差别，毕竟都是卷积，**你给它学什么它就输出什么**，最关键的是三种方法该如何进行标签分配，以及后处理的难易程度。

#### 6.1.3.4. Label Assignment

这部分就着重探讨一下前述的三种角点回归思路。
