...
![](C:\Users\Neo\Desktop\vision_tutorial\Image_base\localfeaturematching.png)
...
![](C:\Users\Neo\Desktop\vision_tutorial\Image_base\imgstitching.png)

<center>特征点检测在图像拼接中的应用</center>
...
![](C:\Users\Neo\Desktop\vision_tutorial\Image_base\detect_invariant.png)
...
上图一共有四组不同$\sigma$大小的对照，每一组实最下方得到的信号是**规范化的LoG**处理原信号后得到的响应。由于斑点的两个边缘的过零点很接近，它们的下侧峰发生重叠直接合二为一，生成一个很强的峰（响应）。观察每组对照的最后一行，我们会发现，不管blob的尺度有多大，**总有一个尺度上的LoG能够对此斑点取得最大的响应，而取得最大响应对应的LoG的尺度，就被称为这个斑点特征的尺度。**换句话说，不论斑点特征属于哪一个尺度，我们都能通过一个对应尺度的标准化LoG”模板“将其检出。
...
![](C:\Users\Neo\Desktop\vision_tutorial\Image_base\secondderivativeofgaussian.png)

再仔细看看上面的图片，再想想为什么要使用积分图？这是因为我们希望用积分图上的矩形来近似替代LoG的计算！若要进行标准化，仍然是计算后乘以尺度的平方。看下图就更清晰了：

![](C:\Users\Neo\Desktop\vision_tutorial\Image_base\boxfilter.png)
...
对于在尺度空间建立的特征点，这个问题在寻找特征点的时候就已经解决了。而对于FAST算子生成的特征点，由于只在一个尺度上检验，因此没有尺度不变性。FAST在寻找特征点的时候没有采用梯度特征而是直接基于像素值进行计算，没有必要也无法建立尺度空间。其解决方法是采用高斯图像金字塔，直接通过降采样，在不同尺度上寻找特征点以尽量覆盖所有大小。虽然这牺牲了一定的准确性和可重复性，但实时性有了极大地提高。
